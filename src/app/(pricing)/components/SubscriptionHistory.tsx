...
import { FilterModal } from './modals/FilterModalSubscription';
...
const SubscriptionHistory = () => {
  ...
  const exportToCSV = () => {
    const headers = [
      'ID', 'User', 'Plan', 'Amount', 'Payment Date',
      'Status', 'Renewal Date'
    ];
    const csvData = subscriptions.map(sub => [
      ...
      sub.planName,
      sub.amount,
      format(new Date(sub.paymentDate), 'dd/MM/yyyy'),
      sub.status,
      format(new Date(sub.renewalDate), 'dd/MM/yyyy'),
    ]);
    ...
    a.href = url;
    a.download = `subscriptions-${format(new Date(), 'dd-MM-yyyy')}.csv`;
    a.click();
  };
